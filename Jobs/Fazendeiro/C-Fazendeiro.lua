...
function CMR_Fazendeiro_PainelDX()
	dxDrawImage(x*950, y*230, x*200, y*250, "img/TelaFundo.png", 0, 0, 0)
	dxDrawText("Fazendeiro", x*990*2, y*250, x*125, y*40, tocolor(255, 255, 255, 255), 1.00, CMR_Font_14, "center", "top", false, false, false, false, false)
	dxDrawImage(x*975, y*300, x*150, y*50, "img/Button.png", 0, 0, 0)
	if not (getElementData(localPlayer, "CMR:FazendeiroTrab") or false) then
...
function CMR_Fazendeiro_Click(_, state)
	if state == "down" then
		if painel_Fazendeiro then
			-- Fechar
			if isCursor(x*975, y*380, x*150, y*50) then
				playSoundFrontEnd(43)
				CMR_Fazendeiro_Painel()
			end

			-- Iniciar/Finalizar
			if isCursor(x*975, y*300, x*150, y*50) then
				playSoundFrontEnd(43)
				triggerServerEvent("CMR:Fazendeiro:IniciarFinalizar", localPlayer, localPlayer)
			end

		end
	end
end
addEventHandler("onClientClick", root, CMR_Fazendeiro_Click)

function CMR_Fazendeiro_Painel()
	if not painel_Fazendeiro then
		addEventHandler("onClientRender", root, CMR_Fazendeiro_PainelDX)
		painel_Fazendeiro = true
		showCursor(true)
	else
		removeEventHandler("onClientRender", root, CMR_Fazendeiro_PainelDX)
		painel_Fazendeiro = false
		showCursor(false)
	end
end
addEvent("CMR:Fazendeiro:PainelDX", true)
addEventHandler("CMR:Fazendeiro:PainelDX", root, CMR_Fazendeiro_Painel)
