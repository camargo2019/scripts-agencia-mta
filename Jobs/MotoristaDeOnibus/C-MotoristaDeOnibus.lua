...
--- 		CAMARGO SCRIPTS  		   ---
	 -----	 AGENCIA BRP		-----
------------------------------------------
local painel_MotoristaDeOnibus = false
...
function CMR_MotoristaDeOnibus_PainelDX()
	dxDrawImage(x*950, y*230, x*200, y*250, "img/TelaFundo.png", 0, 0, 0)
	dxDrawText("MotoristaDeOnibus", x*990*2, y*250, x*125, y*40, tocolor(255, 255, 255, 255), 1.00, CMR_Font_14, "center", "top", false, false, false, false, false)
	dxDrawImage(x*975, y*300, x*150, y*50, "img/Button.png", 0, 0, 0)
	if not (getElementData(localPlayer, "CMR:MotoristaDeOnibusTrab") or false) then
...
function CMR_MotoristaDeOnibus_Click(_, state)
	if state == "down" then
		if painel_MotoristaDeOnibus then
			-- Fechar
			if isCursor(x*975, y*380, x*150, y*50) then
				playSoundFrontEnd(43)
				CMR_MotoristaDeOnibus_Painel()
			end

			-- Iniciar/Finalizar
			if isCursor(x*975, y*300, x*150, y*50) then
				playSoundFrontEnd(43)
				triggerServerEvent("CMR:MotoristaDeOnibus:IniciarFinalizar", localPlayer, localPlayer)
			end

		end
	end
end
addEventHandler("onClientClick", root, CMR_MotoristaDeOnibus_Click)

function CMR_MotoristaDeOnibus_Painel()
	if not painel_MotoristaDeOnibus then
		addEventHandler("onClientRender", root, CMR_MotoristaDeOnibus_PainelDX)
		painel_MotoristaDeOnibus = true
		showCursor(true)
	else
		removeEventHandler("onClientRender", root, CMR_MotoristaDeOnibus_PainelDX)
		painel_MotoristaDeOnibus = false
		showCursor(false)
	end
end
addEvent("CMR:MotoristaDeOnibus:PainelDX", true)
addEventHandler("CMR:MotoristaDeOnibus:PainelDX", root, CMR_MotoristaDeOnibus_Painel)
