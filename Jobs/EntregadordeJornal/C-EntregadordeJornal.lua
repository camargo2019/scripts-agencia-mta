...
--- 		CAMARGO SCRIPTS  		   ---
	 -----	 AGENCIA BRP		-----
------------------------------------------
local painel_EntregadordeJornal = false
...
function CMR_EntregadordeJornal_PainelDX()
	dxDrawImage(x*950, y*230, x*200, y*250, "img/TelaFundo.png", 0, 0, 0)
	dxDrawText("Entregador de Jornal", x*990*2, y*250, x*125, y*40, tocolor(255, 255, 255, 255), 1.00, CMR_Font_12, "center", "top", false, false, false, false, false)
	dxDrawImage(x*975, y*300, x*150, y*50, "img/Button.png", 0, 0, 0)
	if not (getElementData(localPlayer, "CMR:EntregadordeJornalTrab") or false) then
...
function CMR_EntregadordeJornal_Click(_, state)
	if state == "down" then
		if painel_EntregadordeJornal then
			-- Fechar
			if isCursor(x*975, y*380, x*150, y*50) then
				playSoundFrontEnd(43)
				CMR_EntregadordeJornal_Painel()
			end

			-- Iniciar/Finalizar
			if isCursor(x*975, y*300, x*150, y*50) then
				playSoundFrontEnd(43)
				triggerServerEvent("CMR:EntregadordeJornal:IniciarFinalizar", localPlayer, localPlayer)
			end

		end
	end
end
addEventHandler("onClientClick", root, CMR_EntregadordeJornal_Click)

function CMR_EntregadordeJornal_Painel()
	if not painel_EntregadordeJornal then
		addEventHandler("onClientRender", root, CMR_EntregadordeJornal_PainelDX)
		painel_EntregadordeJornal = true
		showCursor(true)
	else
		removeEventHandler("onClientRender", root, CMR_EntregadordeJornal_PainelDX)
		painel_EntregadordeJornal = false
		showCursor(false)
	end
end
addEvent("CMR:EntregadordeJornal:PainelDX", true)
addEventHandler("CMR:EntregadordeJornal:PainelDX", root, CMR_EntregadordeJornal_Painel)
